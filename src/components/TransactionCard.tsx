      ...
      >
        ...
        {transaction.type === "expense" && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              handleNecessityClick(
                transaction.necessity === "need" ? "want" : "need"
              );
            }}
            className={cn(
              "w-2.5 h-2.5 rounded-full shrink-0 transition-all hover:scale-125",
              transaction.necessity === "need" &&
                "bg-need shadow-[0_0_8px_hsl(var(--need)/0.5)]",
              transaction.necessity === "want" &&
                "bg-want shadow-[0_0_8px_hsl(var(--want)/0.5)]",
